...
\begin{document}
	...
	\chapter{Arhitektura rješenja}
		...
		\begin{figure}[H]
			...
			\label{fig:db}
		\end{figure}
		Valja napomenuti da svi entiteti dijele atribute ts\_created, ts\_modified, user\_created, user\_modified radi lakšeg praćenja stvaranja i promjena točnih zapisa unutar tablica, a jer su dijeleni neće biti dodatno spomenuti tijekom detaljnijeg opisa svakog od entiteta. Sami entiteti definirani su korisničkim zahtjevima (definirani u poglavlju \ref{cha:zahtjevi}).
		\begin{table}[H]
		...
		Implementacija Codeflow aplikacije sturkturirana je prema modernoj troslojnoj arhitekturi (spomenuta tijekom početka poglavlja \ref{cha:tehnologije}). Prezentacijski sloj (znan još pod imenom \textit{frontend}), poslovni sloj i podatkovni sloj (znani još kao i \textit{backend}) jasno su vidljivi tijekom pogleda na arhitekturu aplikacije. U narednim podpoglavljima implementacija svakog od spomenutih slojeva objašnjena je.
		\begin{figure}[H]
			\centering
			\includegraphics[width=\linewidth]{pictures/prikazi/Arhitektura.png}
			\caption{Arhitektura Codeflow aplikacije.}
			\label{fig:arh}
		\end{figure}
