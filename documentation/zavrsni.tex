...
\begin{document}
	...
	Internetska stranica koja kombinira elemente socijalnih mreža i elemente stranica s tematikom programskog koda treba omogućiti korisniku ispunjavanje mješavine elementarnih \textbf{funkcionalnih} i \textbf{nefunkcionalnih} zahtjeva koje stranice oba navedena tipa ispunjavaju.
		...
		Većina modernih socijalnih mreža traže od svojih korisnika da prvo naprave korisnički račun prije nego što im dopuste pristup svojim glavnim funkcionalnostima. Analogno tako treba tražiti i Codeflow, stoga na korisnika treba gledati iz dvije različite perspektive; kao na \textbf{neregistriranog korisnika} i \textbf{registriranog korisnika}.
	
			\subsection{Zahtjevi neregistriranog korisnika}
			Neregistriran korisnik, prema uzoru na mnoge moderne socijalne mreže, ima mogućnost pristupanja stranici dobrodošlice te pravo registriranja. Neregistrirani korisnik tijekom registracije treba upisati određene podatke s kojima će se, ako su ispravni, prijaviti. Činom registracije neregistrirani korisnik postaje registriran. Grafički prikaz zahtjeva prikazan je na slici \ref{fig:zahtjevi-nereg}.
			\begin{figure}[H]
				\centering
	...
	\end{itemize}
		...
		\section{HackerRank}
		Internetska stranica HackerRank pruža drugačije usluge od već navedenih stranica. Namjera stranice povezivanje je programera s tvrtkama koje bi bile zainteresirane za njih, ali i učenje novih jezika, programskih algoritama te paradigmi.\\
	...
	\end{itemize}
	U nastavku tehnologije korištene unutar Codeflow aplikacije bit će razmotrene po slojevima kojima, u kontekstu aplikacije, pripadaju.
		...
		Korištene tehnologije poslovnog sloja temeljene su na programskom jeziku Javi. Uz Javu korišten je i Maven, alat za upravljanje projektom koji nudi mogućnosti upravljanja Java ovisnostima \engl{dependencies}. Java ovisnosti već su napisani Java paketi koji proširuju osnovnu Javinu funkcionalnost.
		...
		Tehnologije podatkovnog sloja sastoje od sustava za upravljanja bazom podataka \engl{database} (skraćeno SUBP, engl. \textit{RDBMS}). Interakcija s tim sustavima odvija se korištenjem SQL-a \engl{Structured Query Language}.
			...
			PostgreSql najnapredniji je sustav za upravljanje bazom podataka otvorena koda. Dizajniran je za obradu i pohranu velike količine podataka. Popularan je i zbog toga ga podržavaju mnogi ORM radni okviri (poput Hibernatea).
	...
	\chapter{Arhitektura rješenja}
		...
		Entitet \textbf{test case} unutar baze podataka definiran je relacijom test\_case. Opisuje testni slučaj koji korisnik zadaje tijekom izrade zadatka, a koji se koristi kod dinamičke provjere zadatka. Vlastiti atributi test case entiteta su test\_case\_id, task\_id, input i output. 
		\begin{table}[H]
			\caption{Test Case}
			\label{tbl:testcase}
			\centering
		...
		\end{figure}
			...
			U konačnici, \textbf{podatkovni sloj} koristi Axios biblioteku za slanje zahtjeva te React Reducer funkcionalnost Reacta za strukturirano spremanje i upravljanje podacima unutar React aplikacije.
	 		...
	 		\textbf{Podatkovni sloj} sastoji se od programskih klasa (znane još kao i model) koje opisuju entitete baze podataka te JPA repozitorijska \engl{repositories} sučelja. Repozitorijska sučelja interno unutar svojih implementacija provode prevođenje programskih naredbi u SQL naredbe te redaka dobivenih SQL upitima u objekte definiranih prigodnim modelom. Interna implementacija uvelike ovisi o Hibernate ORM radnom okviru. Podatkovni sloj jedini je sloj koji šalje zahtjeve bazi podataka.
